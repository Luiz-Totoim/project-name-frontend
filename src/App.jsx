  ...
  return (
    <>
      <main id="conteudo" tabIndex={-1}>
        <h1 className="app__title">NewsExplorer — Busca de Notícias</h1>
        <SearchBar onSearch={handleSearch} />
        <div className="demo-actions">
          <Button variant="ghost" onClick={() => setOpenDemo(true)}>
            Abrir Modal de Exemplo
          </Button>
        </div>
        {loading && <Preloader />}
        {error && !loading && (
          <div className="results__status" role="alert">
            {error}
          </div>
        )}
      ...
      <Modal open={openDemo} onClose={() => setOpenDemo(false)} title="Demo UI Kit">
        <p style={{ marginTop: 0 }}>
          Este modal demonstra foco inicial, trap de foco, fechamento por ESC, clique fora e botão fechar.
        </p>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            alert('Form enviado (demo).');
          }}
          noValidate
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              <Button variant="ghost" onClick={() => setOpenDemo(false)}>
                Cancelar
              </Button>
